...
class KMeans {
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    assert(oldMeans.size == newMeans.size)
    (oldMeans zip newMeans).par forall { case (x, y) => x.squareDistance(y) <= eta }
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    if (!isConverged) kMeans(points, newMeans, eta) else newMeans
  }
}
