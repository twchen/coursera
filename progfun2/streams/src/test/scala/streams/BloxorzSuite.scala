...
class BloxorzSuite extends FunSuite {
  ...
  test("neighborsWithHistory should correctly return all valid neighboring blocks") {
    new Level1 {
  ...
  test("newNeighborsOnly should return neighbors that are not explored before") {
    new Level1 {
      val neighbors = Stream(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up)))
